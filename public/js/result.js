...
(async () => {
  ...
  try {
    ...
    const statusMap = {
      active:   ['Active subscriber',    'tx-status tx-status--active'],
      previous: ['Previously connected', 'tx-status tx-status--previous'],
      never:    ['Never connected',      'tx-status tx-status--never'],
    };
    ...
    if (loc.connection_status === 'active') {
      ...
      document.getElementById('sub-connected').textContent = fmtDate(sub.connected_since);
      ...
      if (sub.upgrade_eligible) {
        const card = document.getElementById('upgrade-on');
        card.hidden = false;
